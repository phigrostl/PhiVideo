...
namespace PGR {
    ...
    void Application::RenderVideo() {
        ...
        threads.emplace_back([&frameCounter, &frameNum, &currentFPS, this]() {
            ...
            while (true) {
                ...
                char buf[512];
                sprintf(buf, "\r%.2f%% %d.%d/%d.%d %.2fs/%.2fs(%.2fs) FPS: %d",
                    progress,
    ...
    void Application::Render(float t, Framebuffer* fb, bool drawBack, bool cover) {
        ...
        const float offsetH = m_Width * (0.005f * size + 0.005f);
        ...
        for (size_t i = 0; i < numJudgeLines; i++) {
            ...
            ev.x = (ev.x * m_Width - m_Width / 2.0f) * size + m_Width / 2.0f;
            ev.y = m_Height - ((ev.y * m_Height - m_Height / 2.0f) * size + m_Height / 2.0f);
            evs.push_back(ev);
            ...
            if (DEBUG) {
                ...
                float offsetH = m_Width * (0.004f * size + 0.0055f);
        ...
        for (int i = 0; i < numJudgeLines; i++) {
            ...
            for (size_t j = 0; j < notesCount; j++) {
                ...
                if (DEBUG) {
                    ...
                    const float debugX = (!drawHead ? noteAtlineX : headX) + sin(noteDrawRotateRad) * offsetH;
                    const float debugY = (!drawHead ? noteAtlineY : headY) + cos(noteDrawRotateRad) * offsetH;
        ...
        for (int i = 0; i < numJudgeLines; i++) {
            ...
            for (size_t j = 0; j < notesCount; j++) {
                ...
                if (DEBUG && drawHead) {
                    ...
                    const float debugX = headX + sinDrawRad * offsetH;
                    const float debugY = headY + cosDrawRad * offsetH;
        ...
        for (size_t effectIdx = 0; effectIdx < effectCount; effectIdx++) {
            ...
            ev.x *= m_Width;
            ev.y *= m_Height;
            ev.x = (ev.x - m_Width / 2.0f) * size + m_Width / 2.0f;
            ev.y = m_Height - ((ev.y - m_Height / 2.0f) * size + m_Height / 2.0f);
            ...
            fb->DrawTexture((int)(finalX - halfEffectSize + 0.5f), (int)(finalY - halfEffectSize + 0.5f), img, (int)texScale);
        ...
        if (DEBUG) {
            std::sort(m_Info.chart.data.clickCollection.begin(), m_Info.chart.data.clickCollection.end(), [](const HitFx& a, const HitFx& b) {
            ...
            for (int i = (int)m_Info.chart.data.judgeLines.size() - 1; i >= 0; i--) {
                ...
                if (sx < 0.1f || sx > 0.9f || sy < 0.1f || sy > 0.9f || ev.alpha < 0.0f) {
                    sprintf(Lbuf, "[%d](%.2f, %.2f) : %dd : %.2f : %.2f", i, ev.x, ev.y, (int)ev.rotate, ev.alpha, ev.speed);
        ...
        if (!DEBUG || m_Info.chart.data.judgeLines.size() == 0) {
            ...
            for (size_t i = 0; i < BpmIndexes.size(); i++) {
                ...
                if (BpmIndexes.size() == 1) {
                    sprintf(buf, "%d/%d ",
                        (int)m_Info.chart.data.judgeLines[BpmIndexes[i]].sec2beat(t, m_Info.chart.data.offset),
                        (int)m_Info.chart.data.judgeLines[BpmIndexes[i]].sec2beat(endTime, m_Info.chart.data.offset)
                    );
                } else {
                    sprintf(buf, "[%.2f] %d/%d ",
        ...
        fb->DrawTextTTF(m_Width / 2, (int)(m_Height * 1054.0f / 1080.0f), m_UI.info, Vec4(1.0f, 0.5f), m_Width * 20.0f / 1920.0f, 0.5f);
